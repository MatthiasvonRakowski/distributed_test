...
echo "input,cores,chunk_size,run1s,run1e,run2s,run2e,run3s,run3e,run4s,run4e,run5s,run5e" > runtime.csv
...
do
    ...
    do 
        for chunk_size in 1 10 #16 32 64 128 256 512 1024 2048 4096
        do
            echo -n $inputSize >> runtime.csv
            echo -n "," >> runtime.csv
            ...
            do
                echo -n "," >> runtime.csv
                # to test on any computer (e.g. in EM 2.50 or your laptop)
                printf "%s" "$(srun --partition=amd-longq --cpus-per-task=$cores cabal exec -- haskell-totient 1 $inputSize -s $chunk_size +RTS -N$cores)" >> runtime.csv
                echo -n "," >> runtime.csv
                printf "%s" "$(srun --partition=amd-longq --cpus-per-task=$cores cabal exec -- haskell-totient 1 $inputSize -e $chunk_size +RTS -N$cores)" >> runtime.csv
